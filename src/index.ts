...
import { sendBroadcast, replyToSender, sendToOwner } from './messageHandling';
...
bot.on('message', (ctx) => {
    if (getUser(ctx.from.id, userList) > -1) {
        ...
        if(userList[getUser(ctx.from.id, userList)].activeChannel){
            ...
            let myUser = userList[getUser(ctx.from.id, userList)];
            let myChannel = chanList[getChannel(myUser.activeChannel, chanList)];
            ...
            if (myChannel.owner == ctx.from.id){
                ...
                if (ctx.message.reply_to_message){
                    // message is a reply
                    replyToSender(ctx, userList[getUser(ctx.from.id, userList)], myChannel, userList, chanList);
                } else {
                    // message is a broadcast
                    sendBroadcast(ctx, userList[getUser(ctx.from.id, userList)], myChannel, userList);
                }
            // User has send access to channel
            } else if (myChannel.senders.includes(ctx.from.id)){
                // Send message to channel owner
               sendToOwner(ctx, myUser, myChannel, userList);
